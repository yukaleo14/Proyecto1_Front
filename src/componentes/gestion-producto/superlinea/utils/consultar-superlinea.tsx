...
function SuperLineaForm({ superLinea, onClose, onSuccess }: { superLinea: SuperLinea | null; onClose: () => void; onSuccess: (mensaje: string) => void }) {
  const { register, handleSubmit, reset, setError, formState: { errors, isSubmitting } } = useForm<SuperLineaFormValues>({ resolver: yupResolver(schema) as any, defaultValues: { nombre: "", descripcion: "" } });
